    ...
    <form action="{{ route('products.update',$product->id) }}" method="POST">
        ...
        <div class="col-xs-6 col-sm-6 col-md-6">
            <div class="form-group">
                ...
                <select name="category_id" class="form-control" style="height:39px;">
                            @foreach($categories as $key=>$category)
                            <option <?php if($product->category_id == $category->id ){ ?> selected="selected" <?php } ?>  value="{{$category->id}}">{{$category->name}}</option>
                            @endforeach
                </select>
            </div>
        </div>
